/* Binary searching elements in the array
Name: Shivam Verma
...
class Array1D
{
    ...
    public:
        ...
        template <typename U>
        friend int binarySearch(const Array1D<U> obj, U searchElement);
...
template <typename T>
int binarySearch(const Array1D<T> obj, T searchElement)
{
    int leftIndex = 0, rightIndex = obj.size - 1;
    
    while(leftIndex <= rightIndex)
    {
        int mid = (leftIndex + rightIndex) / 2;

        if(obj[mid] == searchElement)
            return mid;
